...
public class AuthVelocityPlugin {
  ...
  @Inject
  public AuthVelocityPlugin(
      final ProxyServer server, final Logger logger, final @DataDirectory Path dataPath) {
    this.server = server;
    this.logger = logger;
  ...
  public void onProxyInitialize(final ProxyInitializeEvent event) {
    final AuthConfig authConfig =
        configFactory.produceConfig(AuthConfig.class, AUTH_CONFIG_FILE_NAME);
    ...
    final DurationFormatter durationFormatter =
        new DurationFormatter(getPluralizer(POLISH), DEFAULT);

    final AccountFacade accountFacade = getAccountFacade();
    final UserFacade userFacade = getUserFacade(logger, juliet);
    final UserController userController =
        new UserController(logger, server, authConfig, userFacade, accountFacade);
    ...
    final PasswordController passwordController =
        new PasswordController(
            logger, messageSource, userFacade, userController, hashingStrategy, passwordValidator);
    ...
    commands =
        LiteVelocityFactory.builder(server)
